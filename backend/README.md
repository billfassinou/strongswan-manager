...
WebSocket vers `localhost:8080`). Le backend sert `/` (l'app), `/assets/*` et fait un **repli SPA** sur toute route client
...
```bash
make run        # postgres + backend ; API sur http://localhost:8080
```
...
- **Révocation (CRL)** : strongSwan n'expose **pas** de commande VICI pour charger une CRL ;
  ...
  (`CRL_URL`, ex. `http://backend:8080/crl.der`), récupéré par le plugin `curl` de charon.
...
```bash
TOKEN=$(curl -s localhost:8080/api/v1/auth/login -d '{"identity":"admin","password":"admin1234"}' | jq -r .token)
GW=$(curl -s localhost:8080/api/v1/gateways -H "Authorization: Bearer $TOKEN" | jq -r .items[0].id)
curl -s localhost:8080/api/v1/tunnels -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -d '{
